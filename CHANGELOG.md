...
### Fixed
- Fix device detection failure caused by incorrect string split (`'\\n'` literal instead of newline) in `detector.py`
- Fix Ubuntu/Debian install script referencing non-existent `libratbag-tools` package (removed from apt command)
- Fix CRLF (Windows) line endings across all source files causing script execution failures on Linux
- Update README Ubuntu dependency list to match corrected install script
